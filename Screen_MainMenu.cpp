...
Screen_MainMenu::Screen_MainMenu(EventHandler* eventHandler, SDL_Renderer* r, int w, int h, int& cs, Map& map) : Screen(eventHandler, r, w, h) {
	...
	static_img_main_menu_background = IMG_LoadTexture(Game::renderer, "main_menu_background.jpg");
}
...
void Screen_MainMenu::render() {
	...
	SDL_RenderCopy(renderer, static_img_main_menu_background, NULL, &rect);
	//cout <<  rect.w << endl;
	button_start->render();
}
